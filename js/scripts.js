...
(function ($) {
  ...
  function rsubmitForm () {
    ...
    // })
    var settings = {
      url: 'https://api.demo.liaisonstream.com/notification/v1/email/contactUs/request',
      method: 'POST',
      timeout: 0,
      headers: {
        'Content-Type': 'application/json'
      },
      data: JSON.stringify({ email: email, name: name, phoneNumber: phone, requestFor: select })
    }

    $.ajax(settings).done(function (response, err) {
      if (response.statusCode === 'LS001' && response.data && response.data.emailSent) {
        rformSuccess()
      } else {
        rformError()
        rsubmitMSG(false, response.message)
      }
    })
      .fail(function (jqXHR, textStatus, errorThrown) {
        rformError()
        rsubmitMSG(false, jqXHR.responseJSON.message)
      })
  }
